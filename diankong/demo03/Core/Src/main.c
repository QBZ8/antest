...
{
	HAL_UART_Transmit_IT(&huart2,receivedata,3);
	if(receivedata[0]=='0')
	{
		//angle=(int)receivedata[1]*10+(int)receivedata[2];
		angle=((int)receivedata[1]-48)*10+(int)receivedata[2]-48;
		
	}
	else if(receivedata[0]=='1')
	{
		angle=100+((int)receivedata[1]-48)*10+(int)receivedata[2]-48;
...
if(HAL_GetTick() - last_display_time > 100) { // 每50ms更新一次显示
...
if(HAL_GetTick() - last_display_time > 2000) { // 每50ms更新一次显示
   ...
   else if(60<=angle&&angle<=120)
  {
	  ...
	  OLED_PrintString(0,40,flag0,&font16x16,OLED_COLOR_NORMAL);
   if(angle>120)
  {
  ...
  {
	   OLED_NewFrame();
	  duty=(10*angle/(float)ANGLE_MAX+2.5)/100.0*2000;
  __HAL_TIM_SET_COMPARE(&htim4,TIM_CHANNEL_3,duty);

  sprintf(message,"angle:%d",angle);
  
  	static uint32_t last_display_time = 0;
if(HAL_GetTick() - last_display_time > 2000) { 
    last_display_time = HAL_GetTick();
   ...
   else if(60<=angle&&angle<=120)
  {
	  ...
	  OLED_PrintString(0,40,flag1,&font16x16,OLED_COLOR_NORMAL);
   if(angle>120)
  {
	  OLED_PrintString(0,0,"左偏角过大",&font16x16,OLED_COLOR_NORMAL);
