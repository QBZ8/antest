...
// OpenMV UART2 接收变量
volatile uint8_t openmv_rx_byte;
char openmv_rx_buf[16];
volatile uint8_t openmv_rx_len = 0;
volatile uint8_t openmv_number = 0;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        if (openmv_rx_byte == '\r' || openmv_rx_byte == '\n')
        {
            if (openmv_rx_len > 0)
            {
                openmv_rx_buf[openmv_rx_len] = '\0';
                openmv_number = (uint8_t)atoi(openmv_rx_buf);
                openmv_data_ready = 1;
                openmv_rx_len = 0;
            }
        }
        else if (openmv_rx_len < (sizeof(openmv_rx_buf) - 1))
        {
            openmv_rx_buf[openmv_rx_len++] = (char)openmv_rx_byte;
        }
        else
        {
            openmv_rx_len = 0;
        }
        HAL_UART_Receive_IT(&huart2, (uint8_t *)&openmv_rx_byte, 1);
    }
}
...
void mycode_init()
{
    ...
    // 2. 设置工作模式为 Station 模式
    if (Send_Cmd_Wait_Resp_IT(&huart1, "AT+CWMODE=1\r\n", "OK", 2000, 3) != 0) {
        uart_print(&huart2, "ERR: CWMODE\r\n");
    }

    // 3. 连接 Wi-Fi
    sprintf(cmd_buf, "AT+CWJAP=\"%s\",\"%s\"\r\n", WIFI_SSID, WIFI_PASSWORD);
    if (Send_Cmd_Wait_Resp_IT(&huart1, cmd_buf, "OK", 10000, 3) != 0) {
        uart_print(&huart2, "ERR: WIFI\r\n");
    }
    HAL_Delay(2000);

    // 4. 配置 OneNet MQTT 用户信息
    sprintf(cmd_buf, "AT+MQTTUSERCFG=0,1,\"%s\",\"%s\",\"%s\",0,0,\"\"\r\n", DEVICE_NAME, PRODUCT_ID, MQTT_TOKEN);
    if (Send_Cmd_Wait_Resp_IT(&huart1, cmd_buf, "OK", 3000, 3) != 0) {
        uart_print(&huart2, "ERR: MQTT CFG\r\n");
    }

    // 5. 连接 OneNet MQTT 服务器
    sprintf(cmd_buf, "AT+MQTTCONN=0,\"%s\",%d,1\r\n", MQTT_SERVER, MQTT_PORT);
    if (Send_Cmd_Wait_Resp_IT(&huart1, cmd_buf, "OK", 8000, 3) != 0) {
        uart_print(&huart2, "ERR: MQTT CONN\r\n");
    }

    // 6. 订阅发布相关主题
    sprintf(cmd_buf, "AT+MQTTSUB=0,\"$sys/%s/%s/thing/property/post/reply\",0\r\n", PRODUCT_ID, DEVICE_NAME);
    if (Send_Cmd_Wait_Resp_IT(&huart1, cmd_buf, "OK", 5000, 3) != 0) {
        uart_print(&huart2, "ERR: SUBSCRIBE POST REPLY\r\n");
    }

    sprintf(cmd_buf, "AT+MQTTSUB=0,\"$sys/%s/%s/thing/property/set\",0\r\n", PRODUCT_ID, DEVICE_NAME);
    if (Send_Cmd_Wait_Resp_IT(&huart1, cmd_buf, "OK", 5000, 3) != 0) {
        uart_print(&huart2, "ERR: SUBSCRIBE SET\r\n");
    }
