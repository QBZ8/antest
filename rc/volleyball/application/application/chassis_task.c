...
static void chassis_feedback_update(chassis_move_t* chassis_move_update)
{
	...
	for(int i=0;i<4;i++)
	{
		chassis_move_update->motor_chassis[i].speed=chassis_move_update->motor_chassis[i].motor_mesure->speed_rpm * M3508_MOTOR_RPM_TO_VECTOR;
	}
	//移动速度更新
	  chassis_move_update -> vx=(- chassis_move_update ->motor_chassis[0].speed + chassis_move_update ->motor_chassis[1].speed
	                             + chassis_move_update ->motor_chassis[2].speed - chassis_move_update ->motor_chassis[3].speed)*MOTOR_SPEED_TO_CHASSIS_SPEED_VX;
		chassis_move_update -> vy=(- chassis_move_update ->motor_chassis[0].speed - chassis_move_update ->motor_chassis[1].speed
	                             + chassis_move_update ->motor_chassis[2].speed + chassis_move_update ->motor_chassis[3].speed)*MOTOR_SPEED_TO_CHASSIS_SPEED_VY;
		chassis_move_update -> wz=(- chassis_move_update ->motor_chassis[0].speed - chassis_move_update ->motor_chassis[1].speed
	                             - chassis_move_update ->motor_chassis[2].speed - chassis_move_update ->motor_chassis[3].speed)*MOTOR_SPEED_TO_CHASSIS_SPEED_WZ;
}
...
static void chassis_vector_to_mecanum_wheel_speed(const fp32 vx_set, const fp32 vy_set, const fp32 wz_set, fp32 wheel_speed[4])
{
    ...
    wheel_speed[2] = vx_set - vy_set - (-CHASSIS_WZ_SET_SCALE - 0.8f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
    wheel_speed[3] = -vx_set - vy_set -(-CHASSIS_WZ_SET_SCALE - 0.8f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
...
static void chassis_control_loop(chassis_move_t* chassis_move_PID)
{
  ...
  //计算PID
	for(int i=0;i<4;i++)
	{
		PID_calc(&chassis_move_PID->chassis_speed_pid[i],chassis_move_PID->motor_chassis[i].motor_mesure->speed_rpm,chassis_move_PID->motor_chassis[i].speed_set,1);
