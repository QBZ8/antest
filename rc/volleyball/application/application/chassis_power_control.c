...
void chassis_power_control(chassis_move_t *chassis_power_control)
{
    ...
    if(!get_robot_chassis_power_limit() )
        ...
        else
        {
            ...
            if(chassis_power > WARNING_POWER)
            {
                ...
                if(chassis_power < POWER_LIMIT)
                {
                    //scale down
                    //缩小
                      power_scale = (POWER_LIMIT - chassis_power-5) / (POWER_LIMIT - WARNING_POWER+20);
    ...
    {
        total_current += fabs(chassis_power_control->chassis_speed_pid[i].out);
    }
    
    total_current_limit-=4000;
    if(total_current > total_current_limit)
    {
        fp32 current_scale = total_current_limit / (total_current);
            chassis_power_control->chassis_speed_pid[0].out*=current_scale;
						chassis_power_control->chassis_speed_pid[1].out*=current_scale;
						chassis_power_control->chassis_speed_pid[2].out*=current_scale;
						chassis_power_control->chassis_speed_pid[3].out*=current_scale;
    }
}
#endif
    ...
    else if(robot_id == RED_ENGINEER || robot_id == BLUE_ENGINEER || robot_id == 0)
    {
		...
		else
		{
			 ...
			 if(current_power >= WARNING_POWER)
			{
				...
				printf("%.1lf,%.1lf\r\n",total_current_limit,total_current);
