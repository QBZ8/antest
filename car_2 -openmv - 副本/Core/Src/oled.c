 ...
 */
void OLED_Send2(uint8_t *data, uint8_t len)
{
  HAL_I2C_Master_Transmit(&hi2c2, OLED_ADDRESS, data, len, HAL_MAX_DELAY);
  //HAL_I2C_Master_Transmit(&hi2c1, OLED_ADDRESS, data, len, HAL_MAX_DELAY);
}

void OLED_Send1(uint8_t *data, uint8_t len)
{
  //HAL_I2C_Master_Transmit(&hi2c2, OLED_ADDRESS, data, len, HAL_MAX_DELAY);
  HAL_I2C_Master_Transmit(&hi2c1, OLED_ADDRESS, data, len, HAL_MAX_DELAY);
}
 ...
 * @brief 向OLED发送指令
 */
void OLED_SendCmd1(uint8_t cmd)
{
  static uint8_t sendBuffer[2] = {0};
  sendBuffer[1] = cmd;
  OLED_Send1(sendBuffer, 2);
}
void OLED_SendCmd2(uint8_t cmd)
{
  static uint8_t sendBuffer[2] = {0};
  sendBuffer[1] = cmd;
  OLED_Send2(sendBuffer, 2);
}
...
void OLED_Init()
{
  OLED_SendCmd1(0xAE); /*关闭显示 display off*/
  OLED_SendCmd2(0xAE);
  ...
  OLED_SendCmd1(0xAF); /*开启显示 display ON*/
  OLED_SendCmd2(0xAF);
}
...
{
  OLED_SendCmd1(0x8D); // 电荷泵使能
  OLED_SendCmd1(0x14); // 开启电荷泵
  OLED_SendCmd1(0xAF); // 点亮屏幕
	
  OLED_SendCmd2(0x8D); // 电荷泵使能
  OLED_SendCmd2(0x14); // 开启电荷泵
  OLED_SendCmd2(0xAF); // 点亮屏幕
}
...
{
  OLED_SendCmd1(0x8D); // 电荷泵使能
  OLED_SendCmd1(0x10); // 关闭电荷泵
  OLED_SendCmd1(0xAE); // 关闭屏幕
	
  OLED_SendCmd2(0x8D); // 电荷泵使能
  OLED_SendCmd2(0x10); // 关闭电荷泵
  OLED_SendCmd2(0xAE); // 关闭屏幕
}
...
void OLED_SetColorMode(OLED_ColorMode mode)
{
  ...
  {
    OLED_SendCmd1(0xA6); // 正常显示
	OLED_SendCmd2(0xA6); // 正常显示
  }
  if (mode == OLED_COLOR_REVERSED)
  {
    OLED_SendCmd1(0xA7); // 反色显示
	OLED_SendCmd2(0xA7); // 反色显示
  }
}
 ...
 * @note 此函数是移植本驱动时的重要函数 将本驱动库移植到其他驱动芯片时应根据实际情况修改此函数
 */
void OLED_ShowFrame1()
{
  static uint8_t sendBuffer[OLED_COLUMN + 1];
  sendBuffer[0] = 0x40;
  for (uint8_t i = 0; i < OLED_PAGE; i++)
  {
    OLED_SendCmd1(0xB0 + i); // 设置页地址
    OLED_SendCmd1(0x00);     // 设置列地址低4位
    OLED_SendCmd1(0x10);     // 设置列地址高4位
    memcpy(sendBuffer + 1, OLED_GRAM[i], OLED_COLUMN);
    OLED_Send1(sendBuffer, OLED_COLUMN + 1);
  }
}

void OLED_ShowFrame2()
{
  static uint8_t sendBuffer[OLED_COLUMN + 1];
  sendBuffer[0] = 0x40;
  for (uint8_t i = 0; i < OLED_PAGE; i++)
  {
    OLED_SendCmd2(0xB0 + i); // 设置页地址
    OLED_SendCmd2(0x00);     // 设置列地址低4位
    OLED_SendCmd2(0x10);     // 设置列地址高4位
    memcpy(sendBuffer + 1, OLED_GRAM[i], OLED_COLUMN);
    OLED_Send2(sendBuffer, OLED_COLUMN + 1);
  }
}
