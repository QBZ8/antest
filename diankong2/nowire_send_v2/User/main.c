...
uint8_t senderrorData1[32]={0xAA,0x00,0x01,0x1A,
	                        0x8F,0x14,0xE3,0x5D,0xBB,0x9A,0x2C,0x71,0xD9,0x06,
						    0xA7,0x3E,0x82,0x4B,0xF0,0xC5,0x39,0x6E,0x53,0x1D,
						    0xFC,0x28,0x97,0x63,0xAD,0x40,
							0xAB,0xCD};

uint8_t senderrorData2[32]={0xAA,0x00,0x01,0x1A,
	                        0x7B,0xE8,0x15,0xCE,
						    0x22,0x89,0x56,0xB4,0x0F,0x3A,0xD7,0x60,0x9C,0x77,
						    0x4D,0xEA,0x31,0x8B,0x12,0xF9,0x64,0xA3,0x0C,0x5F,
						    0x91,0x7E,
...
uint8_t senderrorData3[32]={0xAA,0x00,0x01,0x1A,
	                        0xC2,0x29,0x54,0xBF,0x18,0xE6,0x43,0xAA,0x35,0x80,
	                        0xFD,0x1A,0x67,0xBE,0x09,0xD2,0x4E,0x95,
						    0x78,0x26,0xB3,0x0A,0xE1,0x5C,0x87,0x3B,
							0xAB,0xCD};

uint8_t senderrorData4[32]={0xAA,0x00,0x01,0x1A,
	                        0x44,0xAF,
						    0x10,0xDB,0x75,0x8E,0x21,0xFA,0x6D,0x34,0x99,0xC6,
						    0x2B,0x50,0xED,0x86,0x1F,0x72,0xBC,0x07,0x48,0xF3,
							0x00,0x00,0x00,0x00,
...
int main(void)
{
	...
	/*OLED显示静态字符串*/
	OLED_ShowString(1, 1, "T:000-000-0sen");		//格式为：T:发送成功计次-发送失败计次-发送标志位
	...
	while (1)
	{
		...
		if (KeyNum == 2)				//K2按下接收与发送的区别在此，接收机发送误码率，发送机发送错误数据，模拟收到干扰导致信号出错
		{
			...
			switch(count)
			{
				case 0:
					for(int i=0;i<32;i++)
				{
					NRF24L01_TxPacket[i]=senderrorData1[i];
				}
				break;
				case 1:
					for(int i=0;i<32;i++)
				{
					NRF24L01_TxPacket[i]=senderrorData2[i];
				}
				break;
				case 2:
					for(int i=0;i<32;i++)
				{
					NRF24L01_TxPacket[i]=senderrorData3[i];
				}
				break;
				case 3:
					for(int i=0;i<32;i++)
				{
					NRF24L01_TxPacket[i]=senderrorData4[i];
				}
				break;
				default:
					break;
		...
		if (ReceiveFlag)				//接收标志位不为0，表示收到了一个数据包
		{
			...
			OLED_ShowNum(4, 14, NRF24L01_RxPacket[4], 2);
