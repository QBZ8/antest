...
//		}
		...
		//rt_tar.st_data.vel_x = *(GetCtrlData());    //头向速度，厘米每秒
		//rt_tar.st_data.vel_y = *(GetCtrlData()+1);    //左向速度，厘米每秒
		rt_tar.st_data.vel_x = 0;    //头向速度，厘米每秒
		rt_tar.st_data.vel_y = 0;    //左向速度，厘米每秒
		rt_tar.st_data.vel_z = *(GetCtrlData()+2);	 //天向速度，厘米每秒
	    rt_tar.st_data.yaw_dps = 0;  //航向转动角速度，度每秒，逆时针为正
		//rt_tar.st_data.yaw_dps = *(GetCtrlData()+3);  //航向转动角速度，度每秒，逆时针为正
		//########################################################################
		//=====
