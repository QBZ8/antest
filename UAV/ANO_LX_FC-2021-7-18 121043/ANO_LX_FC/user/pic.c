...
float pid_solve(pid_param_t *pid, float error)
{
    ...
    // D（带低通滤波）
    float d_raw = (error - pid->pre_error) / dt;
    pid->out_d = pid->low_pass * d_raw + (1 - pid->low_pass) * pid->out_d;
    pid->out_d *= pid->kd;
    ...
    // 输出限幅
    float out =
        MINMAX(pid->out_p, -pid->p_max, pid->p_max) +
        + MINMAX(pid->out_i, -pid->i_max, pid->i_max) +
        MINMAX(pid->out_d, -pid->d_max, pid->d_max);
...
float bangbang_pid_solve(pid_param_t *pid, float error) {
    float BangBang_output = 0, BangBang_error = 2;
    pid->error = error;
